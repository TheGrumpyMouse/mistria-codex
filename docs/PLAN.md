...
| **AB1** | Request board — `request_board.json` shipped form, `/board` by item and by villager, season filter | 212 requests, 193 items, gates in words. Done. |
...
**CI never touches the wiki** — `sources/` is committed, so builds are hermetic, fast, and polite. A separate weekly `refresh-wiki.yml` cron runs the fetcher and opens a PR with the unresolved-token list and coverage delta **in the body**. That PR is the curation to-do list, and reviewing the `sources/` diff is how you notice the wiki changed a fact. It's the maintenance engine of the project.
